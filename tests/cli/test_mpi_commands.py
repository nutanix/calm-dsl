...
class TestMPICommands:
    def test_mpis_list(self):
        runner = CliRunner()
        result = runner.invoke(cli, ["get", "mpis"])
    ...
    def test_mpis_list_with_display_all_flag(self):
        runner = CliRunner()
        result = runner.invoke(cli, ["get", "mpis", "--display_all"])
        assert result.exit_code == 0
        if result.exit_code:
            pytest.fail("MPI list call failed")
        print(result.output)
    ...
    def test_mpi_describe_with_default_version(self):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, ["describe", "mpi", mpi_name])
        assert result.exit_code == 0
        if result.exit_code:
            pytest.fail("MPI list call failed")
        print(result.output)
    ...
    def test_mpi_describe_with_user_version(self):
        ...
        result = runner.invoke(
            cli, ["describe", "mpi", mpi_name, "--version", mpi_version]
        )
        assert result.exit_code == 0
        if result.exit_code:
            pytest.fail("MPI list call failed")
