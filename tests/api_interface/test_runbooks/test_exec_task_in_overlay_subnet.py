...
class TestExecTasksVMEndpoint:
    def setup_method(self):
        ...
        ContextObj.update_project_context(VPC_PROJECT)

        runner = CliRunner()
        self.dsl_linux_endpoint = "Endpoint_Linux_VPC_{}".format(str(uuid.uuid4()))
        result = runner.invoke(
            cli,
            [
                "create",
                "endpoint",
                "--file={}".format(LINUX_EP_PATH),
                "--name={}".format(self.dsl_linux_endpoint),
        ...
        with open(LOCAL_LINUX_ENDPOINT_VPC, "w") as f:
            f.write(self.dsl_linux_endpoint)

        self.dsl_windows_endpoint = "Endpoint_Windows_VPC_{}".format(str(uuid.uuid4()))
        result = runner.invoke(
            cli,
            [
                "create",
                "endpoint",
                "--file={}".format(WINDOWS_EP_PATH),
                "--name={}".format(self.dsl_windows_endpoint),
                "--description='NEW Test DSL Endpoint to delete'",
            ],
        )
        if result.exit_code:
        ...
        with open(LOCAL_WINDOWS_ENDPOINT_VPC, "w") as f:
            f.write(self.dsl_windows_endpoint)
