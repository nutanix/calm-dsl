...
class TestExecTasks:
    ...
    @pytest.mark.parametrize('Runbook', [EndpointMacroOnShell,
                                         ...
                                         LinuxEndpointMacroOnEscript,
                                         WindowsEndpointMacroOnEscript])
    def test_endpoint_macro_in_script(self, Runbook):
        """ test_macro_in_script """
        ...
        endpoint_uuid_for_exec_task = rb['spec']['resources'].get('default_target_reference', {}).get('uuid', '')
        for task in rb['spec']['resources'].get('task_definition_list', []):
            if task['name'] == "ExecTask":
                endpoint_uuid_for_exec_task = task.get('target_any_local_reference', {}).get('uuid', '')

        assert endpoint_uuid_for_exec_task

        endpoint_for_exec_task = None
        for endpoint in endpoint_list:
            if endpoint['uuid'] == endpoint_uuid_for_exec_task:
                endpoint_for_exec_task = endpoint
        ...
        for exec_task in exec_tasks:
            ...
            assert endpoint_for_exec_task['name'] in output_list[0]['output']
            assert endpoint_for_exec_task['type'] in output_list[0]['output']
            assert endpoint_for_exec_task['attrs']['port'] in output_list[0]['output']
            address_in_output = False
            for address in endpoint_for_exec_task['attrs']['values']:
                if address in output_list[0]['output']:
                    address_in_output = True

            assert address_in_output

            if endpoint_for_exec_task['type'] == 'Windows':
                assert endpoint_for_exec_task['attrs']['connection_protocol'] in output_list[0]['output']
