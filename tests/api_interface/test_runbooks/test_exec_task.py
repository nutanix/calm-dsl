...
class TestExecTasks:
    ...
    def test_os_endpoint_macro_in_script(self, Runbook):
        ...
        endpoint_uuid_used = rb['spec']['resources'].get('default_target_reference', {}).get('uuid', '')
        for task in rb['spec']['resources']['runbook'].get('task_definition_list', []):
            if task['name'] == "ExecTask" and task.get('target_any_local_reference', {}).get('uuid', ''):
                endpoint_uuid_used = task.get('target_any_local_reference', {}).get('uuid', '')
        ...
        for exec_task in exec_tasks:
            ...
            cred_username = endpoint_used['attrs']['credential_definition_list'][0]['username']
            assert cred_username in output_list[0]['output']

            if endpoint_used['type'] == 'Windows':
                assert endpoint_used['attrs']['connection_protocol'] in output_list[0]['output']
    ...
    def test_http_endpoint_macro_in_script(self):
        ...
        for exec_task in exec_tasks:
            ...
            assert str(endpoint_used['attrs']['retry_interval']) in output_list[0]['output']
            assert str(endpoint_used['attrs']['retry_count']) in output_list[0]['output']
            assert str(endpoint_used['attrs']['connection_timeout']) in output_list[0]['output']
