...
def create_spec(client):
    ...
    choice = click.prompt(
        "\n{}(y/n)".format(highlight_text("Do you want to specify a destination folder for the VM to be placed in?")),
        default="n",
    )
    ...
    if choice[0] == "y":
        ...
        choice = click.prompt(
            "\n{}(y/n)".format(highlight_text("Do you want to use existing folder in the platform to deploy the new VM?")),
            default="n",
        )
        if choice[0] == "y":
        ...
        choice = click.prompt(
            "\n{}(y/n)".format(highlight_text("Do you want to create new folder or path for the VM? If existing folder path is specified, new path will be created under the existing path.")),
            default="n",
        )
        if choice[0] == "y":
            ...
            delete_empty_folder = click.prompt(
                "\n{}(y/n)".format(highlight_text("Do you want the newly created folder/path to be deleted during application deletion if it does not contain any other resource?")),
                default="n",
            )
