...
def get_policy_extid():
    vpc_name = "@@{clear_counter}@@".split(":")[0]
    policy_name = "@@{clear_counter}@@".split(":")[1]
    priority = "@@{clear_counter}@@".split(":")[2]
...
def create_payload():
    vpc_name = "@@{clear_counter}@@".split(":")[0]
    payload["vpcExtId"] = get_vpc_ext_id(vpc_name)
    if "@@{select_all_policies}@@" == "No":
        payload["routingPolicyExtId"] = get_policy_extid()


def clear_counter():
    create_payload()
    response = session.post(
        "https://{}:{}/api/networking/{}/stats/routing-policies/$actions/clear".format(
            PC_IP, PC_PORT, "@@{flow_virtual_network_api_version}@@"
        ),
        json=payload,
        headers={
