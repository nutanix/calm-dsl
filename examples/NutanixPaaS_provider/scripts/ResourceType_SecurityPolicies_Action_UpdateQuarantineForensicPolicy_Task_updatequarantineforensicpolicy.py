...
def get_policy_extId_eTag(name):
    headers = {"accept": "application/json"}
    params = {"$page": 0, "$limit": 50, "$filter": f"name eq '{name}'"}
    response = session.get(
    ...
    response.raise_for_status()
    extId = [d["extId"] for d in response.json()["data"] if d["name"] == name]

    response1 = session.get(
        "https://{}:{}/api/microseg/{}/config/policies/{}".format(
            PC_IP, PC_PORT, "@@{flow_api_version}@@", extId[0]
        ),
        verify=False,
    )
    response1.raise_for_status()
    return extId[0], response1.headers["ETag"]
...
def check_protocol(proto):
    ...
    if "ALL" in proto or proto == "":
        ...
        proto_type = proto.strip().split(":")[0]
        proto_value = proto.strip().split(":")[1]
    if proto_type == "ALL":
...
def add_intra_group_rule(name):
    # variables
    # 16. app_securedGroup 17. intra_groupAction = DENY / ALLOW
    intra_spec = {
        "spec": {
            "$objectType": "microseg.v4.config.IntraEntityGroupRuleSpec",
            "securedGroupCategoryReferences": get_categorygroup_extId(name),
            "securedGroupAction": "DENY",
        },
        "type": "INTRA_GROUP",
    }
    return intra_spec
...
def create_payload():
    ...
    payload["name"] = "Quarantine Forensic Policy"
    payload["type"] = "QUARANTINE"
    payload[
        "description"
    ] = "System defined quarantine policy for Quarantine:Forensics category"
    ...
    payload["rules"] = []
    payload["rules"].append(add_intra_group_rule("Quarantine:Forensics"))
...
def update():
    ...
    extId, eTag = get_policy_extId_eTag("@@{policy_name}@@")
    print(f'Policy Name:{"@@{policy_name}@@"}, ETag:{eTag}')
    update_security_policy_response = session.put(
        "https://{}:{}/api/microseg/{}/config/policies/{}".format(
    ...
    print(f"create response: {update_security_policy_response.json()}")
    update_security_policy_response.raise_for_status()
    task_uuid = update_security_policy_response.json()["data"]["extId"]
