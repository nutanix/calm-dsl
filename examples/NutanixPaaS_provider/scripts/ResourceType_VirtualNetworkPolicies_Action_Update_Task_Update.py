...
def get_policy_extId():
    vpcextid = payload["vpcExtId"]
    priority = "@@{vpcname}@@".strip().split(":")[2]
    headers = {
        "accept": "application/json",
    }
    ...
    extId = [
        ...
        if d["vpcExtId"] == vpcextid and d["priority"] == int(priority)
    ]

    response1 = session.get(
        "https://{}:{}/api/networking/{}/config/routing-policies/{}".format(
            PC_IP, PC_PORT, "@@{flow_virtual_network_api_version}@@", extId[0]
        ),
        verify=False,
    )
...
def add_basic_details():
    payload["vpcExtId"] = get_vpc_ext_id("@@{vpcname}@@".strip().split(":")[0])
...
def update():
    ...
    get_extid_etag = get_policy_extId()
    update_routing_policy_response = session.put(
        "https://{}:{}/api/networking/{}/config/routing-policies/{}".format(
            PC_IP, PC_PORT, "@@{flow_virtual_network_api_version}@@", get_extid_etag[0]
        ),
        json=payload,
        headers={
            "Content-Type": "application/json",
            "If-Match": get_extid_etag[1],
            "NTNX-Request-Id": str(uuid.uuid4()),
        },
    )
    print(f"create response: {update_routing_policy_response.json()}")
    update_routing_policy_response.raise_for_status()
    task_uuid = update_routing_policy_response.json()["data"]["extId"]
