...
def create_payload():
    ...
    payload.update(
        {
            "name": "@@{vm_name}@@",
            "numSockets": int("@@{num_of_sockets}@@"),
            "numCoresPerSocket": int("@@{num_cores_per_socket}@@"),
            "numThreadsPerCore": int("@@{num_threads_per_core}@@"),
            "memorySizeBytes": int("@@{memory_size_bytes}@@"),
...
def create():
    ...
    create_vm_response = session.post(
    ...
    print(f"response = {create_vm_response.json()}")
    create_vm_response.raise_for_status()
    task_uuid = create_vm_response.json()["data"]["extId"]
