...
payload = {"name": "@@{object_store_name}@@"}
...
def add_public_network_ips():
    if "@@{public_network_ips}@@":
        payload["publicNetworkIps"] = {
            "ipv4": {"value": "@@{public_network_ips}@@", "prefixLength": 32}
        }
...
def update(name):
    ...
    object_details_response = session.get(
        "https://{}:{}/api/objects/{}/operations/object-stores/{}".format(
            PC_IP, PC_PORT, "@@{objects_api_version}@@", object_store_ext_id
        ),
    )
    print(f"object details response: {object_details_response.json()}")
    object_details_response.raise_for_status()
    update_object_storage_response = session.put(
        "https://{}:{}/api/objects/{}/operations/object-stores/{}".format(
            PC_IP, PC_PORT, "@@{objects_api_version}@@", object_store_ext_id
        ),
        json=payload,
        headers={
            ...
            "If-Match": object_details_response.headers["ETag"],
            "NTNX-Request-Id": str(uuid.uuid4()),
        },
    )
    print(f"update response: {update_object_storage_response.json()}")
    update_object_storage_response.raise_for_status()
    task_uuid = update_object_storage_response.json()["data"]["extId"]
    print(f"task_uuid = {task_uuid}")
    if "@@{wait}@@":
        wait(task_uuid)


update("@@{object_store_name}@@")
