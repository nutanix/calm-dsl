...
resp = urlreq(url, verb='GET', auth='BASIC', user=era_user,
              passwd=era_pass, headers=headers)
if resp.ok:
    print("SOFTWARE_PROF_ID={0}".format(json.loads(resp.content)['id']))
else:
    print("Get Software Profile ID request failed",
...
if resp.ok:
    ...
    print("Get Compute Profile ID request failed",
          json.dumps(json.loads(resp.content), indent=4))
    exit(1)
...
if resp.ok:
    ...
    print("Get Network Profile ID request failed",
          json.dumps(json.loads(resp.content), indent=4))
    exit(1)
...
if resp.ok:
    ...
    print("Get DB Parameter ID request failed",
          json.dumps(json.loads(resp.content), indent=4))
    exit(1)

# Set the timestamp to be used throughout the rest of the blueprint
print("TIMESTAMP={0}".format(@@{calm_time("%Y%m%d%H%M")}@@))
