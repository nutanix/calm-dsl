...
CENTOS_PUBLIC_KEY="@@{CENTOS.secret}@@"
...
# Download cfssl to generate self-signed certificates for secured etcd peer-peer communicaion.
...
if [ @@{calm_array_index}@@ -eq 0 ] && [ "${SSL_ON}" == "yes" ];then
  ...
  echo "${CENTOS_PUBLIC_KEY}" | tee ~/.ssh/id_rsa
